...
// Logger - agora acumula para retornar como streams
let debugLogs = [];

function log(title, value = '') {
    const msg = `[*] ${title}${value ? ': ' + value : ''}`;
    debugLogs.push(msg);
    console.log(msg);
}
...
// Converte os logs acumulados em streams de debug para o nuvio
function getDebugStreams() {
    if (debugLogs.length === 0) return [];
    return [{
        name: 'Debug',
        title: debugLogs.join('\n'),
...
async function getStreams(tmdbId, mediaType, season, episode) {
    ...
    for (const slug of guessSlugs) {
        ...
        for (const cdn of cdnList) {
            ...
            if (await testUrl(url)) {
                ...
                log(`DIRECT GUESS ENCONTRADO: ${url}`);
                return [stream, ...getDebugStreams()];
            }
        }
    }
    ...
    // RETORNA STREAMS + DEBUG STREAMS
    return [...finalStreams, ...getDebugStreams()];
}
